...
if uploaded_file is not None:
    ...
    with st.form("confirmar_form"):
        proveedor = st.text_input("Proveedor", label_visibility='visible', label='Proveedor', label_style='color: black; font-weight: bold;', value=proveedor)
        col1, col2 = st.columns(2)
        with col1:
            fecha_input = st.date_input("Fecha", value=datetime.today() if not fecha else datetime.strptime(fecha, "%Y-%m-%d"))
        with col2:
            referencias = st.text_input("Cliente/Referencia", label_visibility='visible', label='Cliente/Referencia', label_style='color: black; font-weight: bold;')
